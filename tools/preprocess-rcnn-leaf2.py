    ...
    for infile in glob.glob(input_folder + "/stoat_fsm_output/ui/*.xml"):
        ...
        if os.path.exists(pngfile) and os.stat(pngfile).st_size > 0 and checkFileValidity(infile):
            ...
            for event, elem in ctx:
                ...
                if widget_name not in rare_targets and len(children) != 0:
                    continue
...
if __name__ == '__main__':
    ...
    # folders_list = glob.glob(a.input_dir + "/*20170510_cleaned_outputs*/incomeincloud.android.activity_246-output")
    ...
    num_processed = 0
    pool = Pool(processes=12, initializer=init, initargs=(countValidFile, train_val, stats))
